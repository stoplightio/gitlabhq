...
Marks a single done todo given by its ID for the current user as pending. The
todo marked as pending is returned in the response.

```
POST /todos/:id/mark_as_pending
```
...
```bash
curl --request POST --header "PRIVATE-TOKEN: 9koXpg98eAheJpvBs5tK" https://gitlab.example.com/api/v4/todos/130/mark_as_pending
```
